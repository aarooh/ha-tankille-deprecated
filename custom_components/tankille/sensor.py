...
from .const import DOMAIN, CONF_STATION_IDS, CONF_LOCATION, CONF_DISTANCE, CONF_LATITUDE, CONF_LONGITUDE
        ...
        try:
            ...
            for fuel_type in station["fuels"]:
                if fuel_type in FUEL_TYPES:
                    entities.append(
                        TankilleFuelPriceSensor(
                            coordinator, station_id, fuel_type
                        )
                    )
        except Exception as err:
...
class TankilleFuelPriceSensor(CoordinatorEntity, SensorEntity):
    ...
    def __init__(
        self, coordinator: TankilleDataUpdateCoordinator, station_id: str, fuel_type: str
    ) -> None:
    ...
    def extra_state_attributes(self) -> Dict[str, Any]:
        ...
        if "address" in station:
            ...
            attrs[ATTR_STATION_ADDRESS] = f"{address.get('street')}, {address.get('city')} {address.get('zipcode')}"
