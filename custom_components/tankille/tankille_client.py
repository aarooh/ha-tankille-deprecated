...
class TankilleClient:
    ...
    async def _auth_async(self):
        ...
        self.token = await self._get_session_token_async({'refreshToken': self.refresh_token})

    async def _get_refresh_token_async(self, login_options: Dict[str, Any]) -> Dict[str, str]:
        ...
        try:
            async with async_timeout.timeout(10):
                ...
                if response.status != 200:
                    ...
                    raise AuthenticationError(f"Error authenticating: {response.status} - {error_text}")
    ...
    async def _get_session_token_async(self, refresh_token_data: Dict[str, str]) -> str:
        ...
        time_since_last_fetch = datetime.datetime.now().timestamp() - self.token_cache['last_fetch']
        
        # 10h cache for access token (expires in 12 hours)
        if time_since_last_fetch <= 36000 and 'accessToken' in self.token_cache.get('data', {}):
        ...
        try:
            async with async_timeout.timeout(10):
                ...
                if response.status != 200:
                    error_text = await response.text()
                    raise AuthenticationError(f"Error refreshing token: {response.status} - {error_text}")
    ...
    async def login(self, email: str, password: str, force: bool = False) -> str:
        ...
        if not force and self.token:
            raise AuthenticationError("Already logged in. Use force=True to login again.")
    ...
    async def get_stations(self) -> List[Dict]:
        ...
        try:
            async with async_timeout.timeout(10):
                ...
                if response.status != 200:
                    error_text = await response.text()
                    raise ApiError(f"Error fetching stations: {response.status} - {error_text}")
    ...
    async def get_stations_by_location(
        ...
        try:
            async with async_timeout.timeout(10):
                ...
                if response.status != 200:
                    error_text = await response.text()
                    raise ApiError(f"Error fetching stations by location: {response.status} - {error_text}")
    ...
    async def get_station(self, station_id: str, days: int = 14) -> Dict:
        ...
        try:
            async with async_timeout.timeout(10):
                ...
                if response.status != 200:
                    error_text = await response.text()
                    raise ApiError(f"Error fetching station: {response.status} - {error_text}")
    ...
    def print_station_info(self, station: Dict) -> None:
        ...
        for price in sorted_prices:
            updated = datetime.datetime.fromisoformat(price['updated'].replace('Z', '+00:00'))
            print(f"  {price['tag']}: {price['price']}€ (updated: {updated.strftime('%Y-%m-%d %H:%M')})")
